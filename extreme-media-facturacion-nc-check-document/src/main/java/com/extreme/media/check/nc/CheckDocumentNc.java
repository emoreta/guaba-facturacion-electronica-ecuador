package com.extreme.media.check.nc;
...
public class CheckDocumentNc {
	
	static final Logger log = Logger.getLogger(CheckDocumentNc.class);
	...
	public static void main(String[] args) {
		...
		DOMConfigurator.configure("C:\\FAC\\log\\LogPropertiesCheckNc.xml");
		Log log = LogFactory.getLog(CheckDocumentNc.class);
		// TODO Auto-generated method stub
		...
		List<ExternalDataNc> listCheck = new ArrayList<ExternalDataNc>();
		...
		try {
			listCheck = q.QueryInfoFactCheckDinamicoNc(con, fecha.trim(),estado.trim());
		...
		for (int y = 0; y < listCheck.size(); y++) {
			...
			try {
				result = qr.QueryInfoDetNc(con, listCheck.get(y).IDENTIFICACION_EMPRESA,
			...
			System.out.println("CLAVE ACCESO:" + listCheck.get(y).clave_acceso_sri_nc);
			...
			try {
				...
				sbS = null;
				sbS = sd.consultaComprobantesWebServices(listCheck.get(y).clave_acceso_sri_nc);
				try {
					Thread.sleep(3000);
				...
				if (infoResSri.get("estado") == null) {
					...
					log.info("RESPUESTA:"+ listCheck.get(y).clave_acceso_sri_nc+"    "+infoResSri.get("estado"));
					if (infoResSri.get("estado").equals("AUTORIZADO")) {
						BufferedWriter writer;
						try {
							writer = new BufferedWriter(new FileWriter(valuePath.trim() + listCheck.get(y).clave_acceso_sri_nc
						...
						try {
							...
							{
							factPdf.GenerarPdfNc(listCheck.get(y), result,parts[2], ambiente, tipoEmision,
									valuePath.trim().trim());
							log.info("CREANDO PDF:"+ listCheck.get(y).clave_acceso_sri_nc);
							}
							else if(parts[3].toString().equals("1"))
							{
								/*factPdf.GenerarPdf(listCheck.get(y), result,parts[2], ambiente, tipoEmision,
										valuePath.trim().trim());*/
								log.info("NOTAS DE CREDITO NO TIENE TEMPLATE PERSONALIZADO");
							}
							else
							{
						...
						log.info("ACTUALIZANDO AUTORIZADAS :"+ listCheck.get(y).clave_acceso_sri_nc);

						try {
							resultUpdate = q.UpdateEstadoCheckNc(con, infoResSri.get("estado"), "",
									infoResSri.get("fechaAutorizacion"), listCheck.get(y).clave_acceso_sri_nc);
							log.info("ACTUALIZADO EN BD AUTORIZADAS :"+ listCheck.get(y).clave_acceso_sri_nc);
						} catch (SQLException e2) {
							// TODO Auto-generated catch block
							e2.printStackTrace();
						...
						// listCheck.get(y).IDENTIFICACION_EMPRESA
							...
							try {
								...
								conmysql.UpdateTableDocuments(fechaUno, listCheck.get(y).IDENTIFICACION_CLIENTE,
										listCheck.get(y).RAZON_SOCIAL_CLIENTE, listCheck.get(y).clave_acceso_sri_nc, "Nota Credito",
										listCheck.get(y).CLAVE_ACCESO + ".pdf", listCheck.get(y).clave_acceso_sri_nc + "_Sign.xml",
										"1", fechaUno, listCheck.get(y).NOMBRE_COMERCIAL,String.valueOf(finalValorTotal));
								
								log.info("INSERTANDO EN MYSQL DOCUMENTO :"+ listCheck.get(y).clave_acceso_sri_nc);
...

					{
						...
						try {
							resultUpdate = q.UpdateEstadoCheckNc(con, infoResSri.get("estado"), "", "",
									listCheck.get(y).clave_acceso_sri_nc);
							log.info("ACTUALIZANDO ESTADO RECIBIDO :"+ listCheck.get(y).clave_acceso_sri_nc);
						} catch (SQLException e1) {
							// TODO Auto-generated catch block
							e1.printStackTrace();
							log.error("ERROR:", e1);
...

					{
						...
						try {
							resultUpdate = q.UpdateEstadoCheckNc(con, infoResSri.get("estado"),
									...
									listCheck.get(y).clave_acceso_sri_nc);
							log.info("ACTUALIZANDO ESTADO DEVUELTA :"+ listCheck.get(y).clave_acceso_sri_nc);
						} catch (SQLException e1) {
							// TODO Auto-generated catch block
							e1.printStackTrace();
							log.error("ERROR:", e1);
...

					{
						...
						try {
							writer = new BufferedWriter(new FileWriter(
									valuePath.trim() + listCheck.get(y).clave_acceso_sri_nc + "_NO_AUTORIZADO" + ".xml"));
							writer.write(sbS.toString());
							writer.close();
							log.info("CREADO ARCHIVO DE NO AUTORIZADOS :"+ listCheck.get(y).clave_acceso_sri_nc);
						} catch (IOException e1) {
							// TODO Auto-generated catch block
							e1.printStackTrace();
							log.error("ERROR:", e1);
						...
						try {
							resultUpdate = q.UpdateEstadoCheckNc(con, infoResSri.get("estado"),
									...
									infoResSri.get("fechaAutorizacion"), listCheck.get(y).clave_acceso_sri_nc);
							log.info("ACTUALIZANDO ESTADO NO AUTORIZADO :"+ listCheck.get(y).clave_acceso_sri_nc);
						} catch (SQLException e1) {
							// TODO Auto-generated catch block
							e1.printStackTrace();
							log.error("ERROR:", e1);
