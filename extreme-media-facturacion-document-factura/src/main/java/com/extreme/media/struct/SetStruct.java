...
public class SetStruct {
	...
	public StructEntity SetConstants(ExternalData ec, List<ExternalDataDetalle> ed) {
		...
		ff.setRuc(ec.IDENTIFICACION_EMPRESA);// EXTRAER DE ARCHIVOS
												// CONFIGURACION
		// ff.setClaveAcceso(ep.fctr_claveaccesofac);
		ff.setClaveAcceso(ec.CLAVE_ACCESO);
		ff.setCodDoc(ec.CODIGO_DOCUMENTO);
		ff.setEstab(ec.ESTABLECIMIENTO);
		ff.setPtoEmi(ec.PUNTO_EMISION);
		// ff.setSecuencial(String.format("%09d", ep.autorizacion));//SE
		// UTILIZARA MOMENTANEAMENTE HASTA QUE PROVEAN LA INFORMACION
		ff.setSecuencial(String.format("%09d", Integer.parseInt(ec.SECUENCIAL)));// SE
																					// UTILIZARA
																					// MOMENTANEAMENTE
																					// HASTA
																					// QUE
		...
		for (int y = 0; y < ed.size(); y++) {
			...
			dc.setCodigoPrincipal(ed.get(y).CODIGO_PRINCIPAL);//SOLICITAR CODIGO DE PRODUCTO
